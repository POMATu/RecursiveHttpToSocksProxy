...
import com.uncensor.Main;
import com.uncensor.Socks;
...
public class ProxyThread implements Callable<Object>, Constants, Closeable {
        ...
        while (socket.isConnected()) {
            ...
            try {
                ...
                for (Socks socks:
                    Main.socks) {
                    try {
                        sendRequest(socks.host, socks.port);
                        ex = null;
                        break;
                    } catch (Exception e) {
                        ex = e;
    ...
    protected void sendRequest(String sockshost, int socksport) throws Exception {
        boolean skipped = false;

        try {
            SocketAddress sa = InetSocketAddress.createUnresolved(sockshost, socksport);
            Proxy proxy = new Proxy(Proxy.Type.SOCKS, sa);
            System.out.println("Using " + sockshost + ":" + socksport);

            String encoding = HeaderEncoding;
            close();
            socket = new Socket(proxy);
            socket.connect(new InetSocketAddress(uriToks[1].toString(), ((Number) uriToks[2]).intValue()), Main.timeout);
            ...
            responseInput = new DataInputStream(new BufferedInputStream(socket.getInputStream()));
                ...
                while (true) {
                    ...
                    for (NameValuePair item :
                            this.responseHeaders) {
                        if (item.getName().toLowerCase().equals("location")) {
                            String loc = item.getValue();

                            for (String fucker :
                                    Main.fuckers) {

                                if (loc.contains(fucker)) {
                                    throw new Exception("Fuck this: " + loc);
                                }
                            }
                        }
                    }
